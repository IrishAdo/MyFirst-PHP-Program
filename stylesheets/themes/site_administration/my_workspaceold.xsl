	...
	<table border="0" cellpadding="5" cellspacing="0" summary="This table holds some information for the digital desktop" width="100%">	
		...
		<xsl:if test="@label">
			<xsl:choose>
				...
				<xsl:otherwise>
					<tr> 
				   		<td valign="top" class="formheader"><xsl:call-template name="get_translation"><xsl:with-param name="check"><xsl:value-of select="@label"/></xsl:with-param></xsl:call-template></td>
						<xsl:if test="cmd">
