...
- Modified $Date: 2005/02/09 12:09:05 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
		...
		<xsl:otherwise>
			<ul class="pageoptions">	
				...
				<xsl:if test="contains(//setting[@name='sp_page_options'] ,'EAF') and //xml_document/modules/module/licence/product/@type='ECMS'">
					<li class="po-eafi"><a class="pagelink"><xsl:attribute name="href">-email-a-friend.php</xsl:attribute>
					<xsl:attribute name="title"><xsl:call-template name="get_translation"><xsl:with-param name="check" select="'LOCALE_EMAIL_THIS_PAGE_NIRVANA'"/></xsl:call-template></xsl:attribute>
					<span class='icon'><span class='text'><xsl:call-template name="get_translation"><xsl:with-param name="check" select="'LOCALE_EMAIL_THIS_PAGE_NIRVANA'"/></xsl:call-template></span></span></a></li>
				</xsl:if>
