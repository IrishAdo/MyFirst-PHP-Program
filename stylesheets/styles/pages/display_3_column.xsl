...
<xsl:template name="display_list">
	...
	<xsl:comment>display three columns title linking only.</xsl:comment>
...
<xsl:for-each select="//modules/container/webobject/module/page[position()!=$title_page][(position() mod 3)=1]">
	<div class='multicolumn'>
		<div class='columncount3'>
			...
			</xsl:choose></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="locations/location[@url=//xml_document/modules/module/setting[@name='script']]"/></xsl:attribute><xsl:value-of select="title" disable-output-escaping="yes"/></a> <xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">PAGE_</xsl:with-param></xsl:call-template>
		</div>
		...
		<div class='columncount3'>
			...
			</xsl:choose></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="following-sibling::page[(position() mod 3) = 1]/locations/location[@url=//xml_document/modules/module/setting[@name='script']]"/></xsl:attribute><xsl:value-of select="following-sibling::page[(position() mod 3) = 1]/title" disable-output-escaping="yes"/></a> <xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">PAGE_</xsl:with-param>
		...
		<div class='columncount3'>		
			[[rightarrow]] <a align='left'><xsl:attribute name="title"><xsl:choose>
				<xsl:when test="following-sibling::page[(position() mod 3) = 2]/metadata/description!=''"><xsl:call-template name="print"><xsl:with-param name="str_value"><xsl:value-of select="translate(following-sibling::page[(position() mod 3) = 2]/metadata/description,'&amp;','')" disable-output-escaping="yes"/></xsl:with-param></xsl:call-template>...</xsl:when>
				<xsl:otherwise><xsl:call-template name="print"><xsl:with-param name="str_value"><xsl:value-of select="following-sibling::page[(position() mod 3) = 2]/title" disable-output-escaping="yes"/></xsl:with-param></xsl:call-template></xsl:otherwise>
			</xsl:choose></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="following-sibling::page[(position() mod 3) = 2]/locations/location[@url=//xml_document/modules/module/setting[@name='script']]"/></xsl:attribute><xsl:value-of select="following-sibling::page[(position() mod 3) = 2]/title" disable-output-escaping="yes"/></a> <xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">PAGE_</xsl:with-param><xsl:with-param name="identifier"><xsl:choose>
			<xsl:when test="//modules/module/setting/licence/product/@type='ECMS'"><xsl:value-of select="following-sibling::page[(position() mod 3) = 2]/@identifier"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="following-sibling::page[(position() mod 3) = 2]/@translation_identifier"/></xsl:otherwise>
			</xsl:choose></xsl:with-param></xsl:call-template>
		</div>
		</xsl:if>
	</div>
