...
- Modified $Date: 2004/09/06 17:35:05 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
<xsl:template name="display_list">
	...
	<xsl:comment>display information in two columns title and summary.</xsl:comment>
	...
	<xsl:for-each select="//xml_document/modules/container/webobject/module/page[position()!=$title_page][(position() mod 2)=1]">
		<div>
			<div style="width:49%;border:0;margin:0;padding:0;display:inline;vertical-align:top">
				<xsl:call-template name="display_this_page">
