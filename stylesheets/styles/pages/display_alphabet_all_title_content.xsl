...
<xsl:template name="display_atoz">
	...
	<xsl:if test="$showinpage = 0">
		...
		<xsl:if test="contains(//setting[@name='real_script'],'index.php')">
		<xsl:for-each select="letters/letter">
			...
			</xsl:choose></xsl:attribute><xsl:attribute name="href"><xsl:value-of select="//setting[@name='base']"/><xsl:value-of select="//setting[@name='script']"/>#atoz_top</xsl:attribute>Top of Page</a>
			</xsl:if>
		</xsl:for-each></xsl:if>
