...
<xsl:template name="display_list">
	...
	<xsl:for-each select="//modules/container/webobject/module/page">
		<xsl:if test="position()!=1">
			<p><a><xsl:attribute name="href"><xsl:value-of select="locations/location[position()=1]"/></xsl:attribute><xsl:value-of select="substring-before(metadata/date[@refinement='publish'],' ')"/> - <xsl:value-of disable-output-escaping="yes" select="title"/></a><br/>
			<xsl:value-of disable-output-escaping="yes" select="summary"/></p>
		</xsl:if>
	</xsl:for-each>
