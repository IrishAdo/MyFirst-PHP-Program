...
- Modified $Date: 2005/01/11 17:15:01 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
<xsl:template name="display_mirror">
	...
	<xsl:if test="//xml_document/modules/container/webobject/module[@name='mirror']">
		...
		<xsl:variable name="realscript"><xsl:value-of select="//setting[@name='real_script']"/></xsl:variable>					
		<!-- <xsl:if test="$realscript != $murl"> -->
		<div id='mirrordata'>
			<xsl:if test="string-length(label)!=0">
				<div class='label'><a><xsl:attribute name="href"><xsl:value-of select="menulocation"/></xsl:attribute><span class='icon'><span class='text'><xsl:value-of disable-output-escaping="yes" select="label"/></span></span></a></div>
			</xsl:if>
				...
				<xsl:choose>
					<xsl:when test="$display_date=1 and contains($display_option,'DATE')">
						...
						<div class="title">
								...
								<a class="news">
									<xsl:attribute name="href"><xsl:value-of select="locations/location[position()=1]"/></xsl:attribute>
								...
								</xsl:call-template> - </span> <xsl:call-template name="print">
							...
							<xsl:otherwise><div class="title"><span class="newsdate"><xsl:call-template name="format_date">
									<xsl:with-param name="current_date"><xsl:value-of select="$page_date"/></xsl:with-param>
									<xsl:with-param name="output_format">DD/MM/YYYY</xsl:with-param>
								</xsl:call-template> - </span><xsl:value-of disable-output-escaping="yes" select="title"/><xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">PAGE_</xsl:with-param></xsl:call-template></div></xsl:otherwise>
						</xsl:choose>
						...
						<div class='contentpos'>
							...
							<xsl:if test="$more=1">
								...
								<a class="readmore">
									<xsl:attribute name="href"><xsl:value-of select="locations/location[position()=1]"/></xsl:attribute>
									<xsl:attribute name="title"><xsl:call-template name="get_translation"><xsl:with-param name="check" select="'LOCALE_LINK_MORE'"/></xsl:call-template> '<xsl:value-of select="title" disable-output-escaping="yes"/>'</xsl:attribute>
									<span class='icon'><span class='text'><xsl:call-template name="get_translation"><xsl:with-param name="check" select="'LOCALE_LINK_MORE'"/></xsl:call-template></span></span>
								</a></div>
							</xsl:if>
						</div>
						</xsl:if>
