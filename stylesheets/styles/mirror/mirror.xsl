...
- Modified $Date: 2004/09/19 10:29:30 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
			...
			<xsl:for-each select="//xml_document/modules/container/webobject/module[@name='mirror']/module/page[position()!=//menu[url=//xml_document/modules/container/webobject/module[@name='mirror']/menulocation]/@title_page][$max_list_count >= position()]">
				...
				<xsl:choose>
					...
					<img>
						...
						<span class="summary"><xsl:value-of select="summary" disable-output-escaping="yes"/></span>
						</xsl:if>
						<xsl:if test="$more=1">
						<div class="readmore">
							...
							<xsl:choose>
								<xsl:when test="string-length(title)>20">More on <xsl:value-of select="translate(substring(title,0,20),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/>...</xsl:when>
						...
						<img alt=""><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/hr.gif</xsl:attribute></img>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
...
<xsl:otherwise>
		<table border="0" cellspacing="1" cellpadding="3" summary="This table contains documents that have been mirrored from another location.." class="mirrortable">
				...
				<xsl:choose>
					...
					<img>
						...
						<xsl:choose>
							...
							<xsl:when test="$title_link=1 or $summary=0">
								<a class="news">
									<xsl:attribute name="href"><xsl:value-of select="locations/location[position()=1]"/></xsl:attribute><xsl:call-template name="print">
							<xsl:with-param name="str_value"><xsl:value-of disable-output-escaping="yes" select="title"/></xsl:with-param>
						</xsl:call-template></a>
								<xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">PAGE_</xsl:with-param></xsl:call-template>
							</xsl:when>
					...
					<xsl:otherwise>
						...
						<xsl:if test="$more=1 and contains($display_option,'READMORE')">
						<div class="readmore">
							<a class="readmore"><xsl:attribute name="href"><xsl:value-of select="locations/location[position()=1]" disable-output-escaping="yes"/></xsl:attribute>
							<xsl:attribute name="title">More on <xsl:value-of select="title"/></xsl:attribute>
							<xsl:choose>
								<xsl:when test="string-length(title)>20">More on <xsl:value-of select="translate(substring(title,0,20),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/>...</xsl:when>
								<xsl:otherwise>More on <xsl:value-of select="translate(title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')"/></xsl:otherwise>
