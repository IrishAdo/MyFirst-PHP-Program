...
<!--												<xsl:when test="$can_buy=0"></xsl:when>-->
		...
		<xsl:if test="$ards!='2'">
			...
			<xsl:for-each select="content/info[@list=$directory_identifier]/results/entry">
				...
				<!--  <input class="button" type="submit" name="back" value="Return" onclick="history.back()">
				</input>-->
		</xsl:otherwise>
	</xsl:choose>
</xsl:template>
	...
	<xsl:for-each select="field">
		...
		<xsl:if test="@id='__add_to_basket__'">
			<xsl:choose>
				...
				<xsl:otherwise><a class='basketadd'><xsl:attribute name="href">_add-to-cart.php?identifier=<xsl:value-of select="$entry_identifier"/>&amp;type=<xsl:value-of select="$entry_type"/></xsl:attribute><span class='icon'><span class='text'><xsl:value-of select="$info_add_label"/></span></span></a></xsl:otherwise>
			</xsl:choose>
		</xsl:if>			
