...
<xsl:template name="display_directory_atoz">
	...
	<xsl:variable name="prodcode"><xsl:value-of select="prodcode"/></xsl:variable>
	<xsl:variable name="prodname"><xsl:value-of select="prodname"/></xsl:variable>
					...
					<div class="featureEntry">
						<li class="storyitem">
							<xsl:call-template name="display_entry">
								...
								<xsl:with-param name="prodcode">INFORMATION</xsl:with-param>
								<xsl:with-param name="prodname">INFORMATION</xsl:with-param>
...
<xsl:template name="display_directory">
	...
	<xsl:choose>
		<xsl:when test="$cat=''">
			<ul><xsl:attribute name='class'>columns<xsl:value-of select="$dcolumn"/></xsl:attribute>
			<xsl:for-each select="category">
				<li><a><xsl:attribute name="href"><xsl:value-of select="$uri"/><xsl:value-of select="uri"/></xsl:attribute><xsl:value-of select="label"/>
		...
		<xsl:otherwise>
		<div><xsl:attribute name='class'>prodcatlist</xsl:attribute>
		<xsl:for-each select="//categorylist[@parent=$cat]/category">
        <div class="folder"><a><xsl:attribute name="href"><xsl:choose>
				<xsl:when test="substring($uri,string-length($uri),1)!='/' and substring(uri,1,1)!='/'"><xsl:value-of select="$uri"/>/</xsl:when>
				<xsl:otherwise><xsl:value-of select="$uri"/></xsl:otherwise>
			</xsl:choose><xsl:value-of select="uri"/></xsl:attribute>
			
			

			<xsl:if test="label='Bedroom Furniture'"><div id="cat1"></div></xsl:if>
			<xsl:if test="label='Dining Furniture'"><div id="cat2"></div></xsl:if>
			<xsl:if test="label='Kitchen Furniture'"><div id="cat3"></div></xsl:if>
			<xsl:if test="label='Bunk Beds'"><div id="cat4"></div></xsl:if>
			<xsl:if test="label='Metal Beds'"><div id="cat5"></div></xsl:if>
			<xsl:if test="label='Occasional Furniture'"><div id="cat6"></div></xsl:if>
			<xsl:if test="label='Wooden Headboards'"><div id="cat7"></div></xsl:if>
			<xsl:if test="label='Metal Headboards'"><div id="cat8"></div></xsl:if>
			<xsl:if test="label='Wooden Beds'"><div id="cat9"></div></xsl:if>
		...
		<xsl:if test="$ards!='2'">
			...
			<xsl:for-each select="content/info[@list=$directory_identifier]/results/entry">
				...
				<xsl:call-template name="display_entry">
					...
					<xsl:with-param name="prodcode"><xsl:value-of select="$prodcode"/></xsl:with-param>
					<xsl:with-param name="prodname"><xsl:value-of select="$prodname"/></xsl:with-param>
					<xsl:with-param name="ards"><xsl:value-of select="$ards"/></xsl:with-param>
				</xsl:call-template>
				</div>
			</xsl:for-each>
...
<xsl:template name="display_entry">
	...
	<xsl:param name="prodcode"></xsl:param>
	<xsl:param name="prodname"></xsl:param>
	...
	<xsl:if test="$ards!='1'">
		<div class='panel'><xsl:attribute name="title"><xsl:value-of select="$real_id"/></xsl:attribute>
		<xsl:for-each select="//content/info[@list=$directory_identifier]/display/seperator_row">
			<xsl:call-template name="display_entry_row">
				<xsl:with-param name="len"><xsl:value-of select="$len"/></xsl:with-param>
				<xsl:with-param name="fake_path"><xsl:value-of select="$fake_path"/></xsl:with-param>
...
<xsl:template name="display_entry_cell">
	...
	<xsl:for-each select="field">
		...
		<xsl:if test="@id='__add_to_enquire__'">
			<xsl:choose>
				<xsl:when test="$can_buy=0"></xsl:when>
				<xsl:when test="$quantity=0"></xsl:when>
				<xsl:otherwise><a><xsl:attribute name="href">balmoral-furniture/enquire/index.php?prodcode=<xsl:value-of select="$prodcode"/>&amp;prodname=<xsl:value-of select="$prodname"/></xsl:attribute><span class='icon'><span class='text'>Enquire</span></span></a></xsl:otherwise>
			</xsl:choose>
		</xsl:if>			
