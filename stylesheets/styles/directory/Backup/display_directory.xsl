...
<!--												<xsl:when test="$can_buy=0"></xsl:when>-->
										...
										<xsl:otherwise>
											...
											<xsl:choose>
												...
												<xsl:otherwise>
													<xsl:choose>
														<xsl:when test="@link=1 and //content/info[@list=$directory_identifier]/display/@summary_only=0"><a><xsl:attribute name='href'><xsl:value-of select="//setting[@name='fake_script']"/>/<xsl:value-of select="//content/info[@list=$directory_identifier]/results/entry[position()=$pos]/field[@name='uri']/value"/></xsl:attribute><xsl:value-of select="$content"/></a></xsl:when>
...
-->
										...
										<xsl:value-of select="//content/info[@list=$directory_identifier]/results/entry[@identifier=$entry_identifier]/field[@name=$name]"/></a></xsl:otherwise>
										</xsl:choose>
									</xsl:when>
