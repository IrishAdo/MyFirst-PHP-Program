...
- Modified $Date: 2005/02/03 08:05:23 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
<xsl:template match="form">
	...
	<xsl:if test=".//@type='file'"><xsl:attribute name="enctype">multipart/form-data</xsl:attribute></xsl:if>
	<xsl:choose>
		<xsl:when test="@method"><xsl:attribute name="method"><xsl:value-of select="@method"/></xsl:attribute></xsl:when>
		<xsl:otherwise><xsl:attribute name="method">post</xsl:attribute></xsl:otherwise>
	</xsl:choose>
	<div>
	...
	<xsl:choose>
		...
		<table border="0" cellpadding="0" cellspacing="0" summary="This table holds a form">
			<xsl:choose>
				...
				<xsl:when test="@width"><xsl:attribute name="class">width<xsl:choose>
			...
			<tr>
			   	<td valign="top" class="TableBackground"><table border="0" cellpadding="3" cellspacing="0" width="100%" summary="This table holds the row information for the forms">
				<xsl:choose>
				<xsl:when test="$show_label=1">
				</xsl:when>
				<xsl:otherwise>
					<tr>
					   	<td valign="top" colspan="2" class="TableHeader"><span>
						<xsl:if test="$title_bullet=1"><img width="20" ><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/title_bullet.gif</xsl:attribute></img>[[nbsp]]</xsl:if>
				...
				<xsl:if test="//@error=1">
					<tr>
					   	<td valign="top" colspan="2" class="tablecell">error</td>
			  		</tr>
				</xsl:if>
				<xsl:choose>
					<xsl:when test="seperator_row/seperator">
						...
						<tr>
							<td class="tablecell"><table summary="new column" border="0"><tr><xsl:apply-templates>
					...
					<xsl:for-each select="./child::*">
						...
						<xsl:choose>
				    		<xsl:when test="$labelinnewrow=1 and (@type!='hidden' or @type!='submit' or @type!='button')">
								...
								<tr>
									<td class="tablecell"><table summary="new column" border="0"><tr><xsl:apply-templates select=".">
												<xsl:with-param name="labelinnewrow"><xsl:value-of select="$labelinnewrow"/></xsl:with-param>
											</xsl:apply-templates>
										</tr>
									</table></td>
...
<xsl:template name="display_submit_buttons">
	<xsl:param name="submit_align">center</xsl:param>
		<tr>
		   	<td valign="top" colspan="2"><xsl:attribute name="align"><xsl:value-of select="$submit_align"/></xsl:attribute>
	  	 	<xsl:for-each select="input">
				...
				<xsl:if test="@type='button'">
					<xsl:choose>
						<xsl:when test="$form_button_type='IMAGE'">
							<a>
								<xsl:attribute name="href"><xsl:value-of select="@command"/></xsl:attribute><img >
							   	<xsl:attribute name="src"><xsl:value-of select="$image_path"/>/button_<xsl:value-of select="@iconify"/>.gif</xsl:attribute>
							   	<xsl:attribute name="alt"><xsl:call-template name="get_translation">
									<xsl:with-param name="check" select="@value"/>
								</xsl:call-template></xsl:attribute>
							   	<xsl:attribute name="name"><xsl:value-of select="@name"/></xsl:attribute>
						   	</img></a><img src="/libertas_images/themes/1x1.gif" width="5" height="5" alt=""/>
						</xsl:when>
						<xsl:when test="$form_button_type='HTML_WITH_ARROWS'">
							<li><xsl:attribute name="class">buttondata</xsl:attribute><a>
								<xsl:attribute name="href"><xsl:value-of select="@command"/></xsl:attribute>
								<xsl:attribute name="class">button</xsl:attribute><strong>&gt;</strong>[[nbsp]]<xsl:call-template name="get_translation">
									<xsl:with-param name="check"><xsl:choose>
										<xsl:when test="@alt!=''"><xsl:value-of select="@alt"/></xsl:when>
										<xsl:when test="@value!=''"><xsl:value-of select="@value"/></xsl:when>
										<xsl:otherwise><xsl:value-of select="."/></xsl:otherwise>
									</xsl:choose></xsl:with-param>
								</xsl:call-template>[[nbsp]]<strong>&lt;</strong></a></li>
						</xsl:when>
						<xsl:otherwise>
							<li><xsl:attribute name="class">buttondata</xsl:attribute><a>
								<xsl:attribute name="href"><xsl:value-of select="@command"/></xsl:attribute>
								<xsl:attribute name="class">button</xsl:attribute>[[nbsp]]<xsl:call-template name="get_translation">
								<xsl:with-param name="check"><xsl:choose>
									<xsl:when test="@alt!=''"><xsl:value-of select="@alt"/></xsl:when>
									<xsl:when test="@value!=''"><xsl:value-of select="@value"/></xsl:when>
									<xsl:otherwise><xsl:value-of select="."/></xsl:otherwise>
								</xsl:choose></xsl:with-param>
							</xsl:call-template>[[nbsp]]</a></li>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
		   	</xsl:for-each></td>
...
<xsl:template match="display_format"></xsl:template>
...
<xsl:template match="radio">
	<xsl:param name="labelinnewrow">0</xsl:param>
		...
		<table summary="List of radio options">
	   	<xsl:for-each select="option">
		<tr><td valign="top"><input class="checkbox" type="radio">
			...
			<xsl:otherwise>
				<xsl:if test="not(../option/@checked) and position()=1"><xsl:attribute name="checked">checked</xsl:attribute></xsl:if>
			</xsl:otherwise>
			</xsl:choose>
			<xsl:if test="../../@onclick">
				<xsl:attribute name="onclick">javascript:<xsl:value-of select="../../@onclick"/>_group(this,'<xsl:value-of select="../@tag"/>');</xsl:attribute>
...
<xsl:param name="entry_identifier"></xsl:param>
	<tr><td class="tablecell"><table class="width100percent" border="0" summary="new row of field for this form"><tr><xsl:apply-templates><xsl:with-param name="entry_identifier"><xsl:value-of select="$entry_identifier"/></xsl:with-param></xsl:apply-templates></tr></table></td></tr>
</xsl:template>
<xsl:template match="seperator">
<xsl:param name="entry_identifier"></xsl:param>
	<td valign="top"><table summary="new column of field for this form" class="width100percent" border="0"><xsl:apply-templates><xsl:with-param name="entry_identifier"><xsl:value-of select="$entry_identifier"/></xsl:with-param></xsl:apply-templates></table></td>
</xsl:template>
