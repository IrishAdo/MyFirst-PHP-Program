...
<xsl:template match="form">
	...
	<xsl:choose>
		...
		<table border="0" cellpadding="0" class="tablebg" cellspacing="0" summary="This table holds a form">
			<xsl:if test="@width">
		...
		<tr>
		   	<td valign="top" colspan="2" class="TableHeader"><span><xsl:if test="$title_bullet=1"><img width="20" border="0"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/title_bullet.gif</xsl:attribute></img>[[nbsp]]</xsl:if><xsl:call-template name="get_translation"><xsl:with-param name="check"><xsl:choose>
		...
		<xsl:choose>
			...
			<tr><td class="tablecell"><table summary="new column" border="0"><tr><xsl:apply-templates>
		...
		<xsl:otherwise><table border="0" cellpadding="3" class="tablebg" cellspacing="0" summary="This table holds a form">
			<xsl:if test="@width">
				<xsl:attribute name="width"><xsl:value-of select="@width"/></xsl:attribute>
			</xsl:if>
			<xsl:choose>
