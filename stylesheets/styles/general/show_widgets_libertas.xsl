...
<xsl:template name="show_containers">
	...
	<xsl:param name="show_content_tag">1</xsl:param>
	<xsl:comment><xsl:value-of select="$displayLayout"/></xsl:comment>	
	<xsl:if test="$display_position='2' and $show_containers=-1 and $show_content_tag=1"><div id="content"><xsl:comment>jump to top links here</xsl:comment></div></xsl:if>
	...
	<xsl:for-each select="//modules/container[@pos=$display_position]">
		...
		<div class="container">
			<xsl:attribute name="id"><xsl:choose>
				<xsl:when test="$display_position='header' or $display_position='footer'"><xsl:value-of select="$display_position"/></xsl:when>
				<xsl:otherwise>position<xsl:value-of select="$display_position"/></xsl:otherwise>
			</xsl:choose>-<xsl:value-of select="position()"/></xsl:attribute>
...
<xsl:for-each select="//modules/container[@identifier = $display_container]/webobject[position() mod $width = $first_column_id]">
	...
	<div ><xsl:attribute name='class'>columncount<xsl:value-of select="../@columns"/></xsl:attribute>
	<xsl:attribute name='id'>position<xsl:value-of select="$display_position"/>-count-<xsl:for-each select="//modules/container[@pos = $display_position]/webobject"><xsl:if test="@identifier = $column_1"><xsl:value-of select="position()"/></xsl:if></xsl:for-each></xsl:attribute>
	<xsl:if test="property/option[name='text-align']/value!='' or property/option[name='width']/value!=''">
	<xsl:attribute name="style"><xsl:if test="property/option[name='text-align']/value!=''">text-align:<xsl:value-of select="property/option[name='text-align']/value"/>;</xsl:if><xsl:if test="property/option[name='width']/value!='' and property/option[name='width']/value!='100%'">width:<xsl:value-of select="property/option[name='width']/value"/>;</xsl:if></xsl:attribute>
	</xsl:if>
	<!--		[<xsl:value-of select="$display_container"/>,<xsl:value-of select="$display_position"/>,<xsl:value-of select="position()"/>] -->
...
<!--		<xsl:if test="following-sibling::webobject[position() mod $width)=$second_column_id]">-->
		<xsl:if test="number($width) >= 2 ">
	<div ><xsl:attribute name='class'>columncount<xsl:value-of select="../@columns"/></xsl:attribute>
	<xsl:if test="following-sibling::webobject/property/option[name='text-align']/value!='' or following-sibling::property/option[name='width']/value!=''">
	<xsl:attribute name="style">
		<xsl:if test="following-sibling::webobject/property/option[name='text-align']/value!=''">text-align:<xsl:value-of select="following-sibling::webobject/property/option[name='text-align']/value"/>;</xsl:if>
		<xsl:if test="following-sibling::webobject/property/option[name='width']/value!='' and following-sibling::webobject/property/option[name='width']/value!='100%'">width:<xsl:value-of select="following-sibling::webobject/property/option[name='width']/value"/>;</xsl:if></xsl:attribute>
	</xsl:if>
		...
		<xsl:if test="$width >= 3 ">
	<div ><xsl:attribute name='class'>columncount<xsl:value-of select="../@columns"/></xsl:attribute>
	<xsl:if test="following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='text-align']/value!='' or following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='width']/value!='' ">
	<xsl:attribute name="style"><xsl:if test="following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='text-align']/value!=''">text-align:<xsl:value-of select="following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='text-align']/value"/>;</xsl:if><xsl:if test="following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='width']/value!='' and following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='width']/value!='100%'">width:<xsl:value-of select="following-sibling::webobject[position() mod $width = $second_column_id]/property/option[name='width']/value"/>;</xsl:if></xsl:attribute></xsl:if>
	...
	<div ><xsl:attribute name='class'>columncount<xsl:value-of select="../@columns"/></xsl:attribute>
	<xsl:if test="property/option[name='text-align']/value!='' or property/option[name='width']/value!='' ">
	<xsl:attribute name="style"><xsl:if test="property/option[name='text-align']/value!=''">text-align:<xsl:value-of select="property/option[name='text-align']/value"/>;</xsl:if><xsl:if test="property/option[name='width']/value!=''">width:<xsl:value-of select="property/option[name='width']/value"/>;</xsl:if></xsl:attribute>
	</xsl:if>
...
<!--
	...
	[<xsl:value-of select="@type"/>,<xsl:value-of select="$show_label"/>]
	[<xsl:value-of select="local-name()"/>,<xsl:value-of select="@identifier"/>,<xsl:value-of select="count(property/option)"/> ,<xsl:value-of select="$position"/>]
-->
	<xsl:choose>
		<xsl:when test="@type='0'">
			...
			<xsl:choose>
				<xsl:when test="$intable=1"><xsl:call-template name="display_a_table">
					...
					<xsl:with-param name="content">0<xsl:call-template name="extract_form_data">
					<xsl:with-param name="cdata"><xsl:value-of select="data" disable-output-escaping="yes"/></xsl:with-param>
				</xsl:call-template></xsl:with-param>
				</xsl:call-template></xsl:when>
				<xsl:otherwise><div class='webobjectcontent'><xsl:call-template name="extract_form_data">
					<xsl:with-param name="cdata"><xsl:value-of select="data" disable-output-escaping="yes"/></xsl:with-param>
				</xsl:call-template></div></xsl:otherwise>
			</xsl:choose></xsl:when>
		<xsl:when test="@type=2">
			...
			<xsl:attribute name='style'><xsl:for-each select="property/option[./name!='label']"><xsl:value-of select="name"/>:<xsl:value-of select="value"/>;</xsl:for-each></xsl:attribute>
			   	<xsl:choose>
							...
							<xsl:when test="module/@name='newsletter'">
								<div class='webobjectcontent'>
									<div class='newsletter_form'>
										<xsl:call-template name="display_form">
											<xsl:with-param name="extract_pos"><xsl:value-of select="../@pos"/></xsl:with-param>										
											<xsl:with-param name="id">newsletter_subscription_form</xsl:with-param>
										</xsl:call-template>
									</div>
								</div>									
							</xsl:when>							
							...
							<xsl:when test="command='WEBOBJECTS_SHOW_SITE_UPDATED_DATE'"><xsl:choose>
								...
								<xsl:otherwise><div><xsl:attribute name='class'><xsl:choose>
									<xsl:when test="$uses_class!=''"><xsl:value-of select="$uses_class"/></xsl:when>
									<xsl:otherwise>date</xsl:otherwise>
								</xsl:choose></xsl:attribute><div class="siteupdatemsg">Site last updated</div><div class="siteupdatedate"><xsl:value-of select="//setting[@name='site_updated']"/></div></div></xsl:otherwise>
							</xsl:choose></xsl:when>
							...
							<xsl:when test="command='WEBOBJECTS_SHOW_MAIN_MENU'"><xsl:attribute name="id">menu</xsl:attribute><xsl:call-template name="display_menu"/></xsl:when>
							<xsl:when test="command='WEBOBJECTS_SHOW_SUB_MENU'"><xsl:attribute name="id">submenu</xsl:attribute><xsl:call-template name="display_submenu"/></xsl:when>
...
<!--
						<xsl:if test="($show_label=1 and $showit=1) or ../property/option[name='label']/value!='' or label/@show='1'">
-->
							...
							<xsl:when test="@name='layoutimage' and @display='image'">
								<xsl:for-each select="file">
									<img class='locationimage'>
										...
										<xsl:attribute name="style">margin:0;padding:0;width:<xsl:value-of select="width"/>;height:<xsl:value-of select="height"/>;border:0;</xsl:attribute>
									</img>
								</xsl:for-each>
							</xsl:when>
							...
							<xsl:when test="@name='mirror'"><xsl:choose>
								<xsl:when test="$intable=1"><xsl:call-template name="display_a_table">
									<xsl:with-param name="header"><xsl:choose>
										...
										<xsl:otherwise><xsl:value-of select="//xml_document/modules/container/webobject/module[@name='mirror']/label"/> <xsl:call-template name="show_edit_button"><xsl:with-param name="cmd_starter">MIRROR_</xsl:with-param></xsl:call-template></xsl:otherwise>
									</xsl:choose></xsl:with-param>
