...
<xsl:template match="form">
	...
	<xsl:choose>
		...
		<tr>
		   	<td valign="top" colspan="2" class="TableHeader"><xsl:if test="$title_bullet=1"><img width="20" border="0"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/title_bullet.gif</xsl:attribute></img>[[nbsp]]</xsl:if><xsl:call-template name="get_translation"><xsl:with-param name="check"><xsl:choose>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
		    		...
		    		<xsl:if test="$labelinnewrow=1 and @type!='hidden'">
						...
						<tr>
					   		<td valign="top"><xsl:apply-templates select="."><xsl:with-param name="labelinnewrow"><xsl:value-of select="$labelinnewrow"/></xsl:with-param></xsl:apply-templates></td>
		    			</tr>
					</xsl:if>
	...
	<xsl:param name="submit_align">center</xsl:param>
		<tr>
	  	 	...
	  	 	<xsl:for-each select="input">
				...
				<a class="button">
					<xsl:attribute name="href"><xsl:value-of select="//xml_document/modules/module[@name='system_prefs' and @display='settings']/setting[@name='real_script']"/>?command=<xsl:value-of select="@command"/></xsl:attribute><xsl:choose>
					<xsl:when test="$form_button_type='IMAGE'">
						<img border="0">
					   	<xsl:attribute name="src"><xsl:value-of select="$image_path"/>/button_<xsl:value-of select="@iconify"/>.gif</xsl:attribute>
					   	<xsl:attribute name="alt">
					...
					<xsl:when test="$form_button_type='HTML_WITH_ARROWS'">
					 [[rightarrow]] <xsl:call-template name="get_translation"><xsl:with-param name="check">
									<xsl:choose>
									<xsl:when test="@alt!=''"><xsl:value-of select="@alt"/></xsl:when>
									<xsl:when test="@value!=''"><xsl:value-of select="@value"/></xsl:when>
									<xsl:otherwise><xsl:value-of select="."/></xsl:otherwise>
				...
				</xsl:choose></a><img src="/libertas_images/themes/1x1.gif" width="5" height="5" alt=""/></xsl:if>
		   	</xsl:for-each></td>
  		</tr>
</xsl:template>
...
<xsl:template match="form_subject">
	<xsl:param name="labelinnewrow">0</xsl:param>
   	<tr class="TableCell">
	   	<td colspan="2" valign="top"><xsl:call-template name="get_translation"><xsl:with-param name="check" select="@label"/></xsl:call-template><xsl:if test="@required"><span class="required">*</span></xsl:if></td>
	</tr>
   	<tr class="TableCell">
	<td colspan="2">
	<xsl:variable name="name"><xsl:value-of select="@name"/></xsl:variable>
	<xsl:choose>
		<xsl:when test="@type='radio'">
			<table class="width100percent" summary="List of subjects lines please select one">
		...
		<xsl:when test="@type='checkbox'">
			<table summary="List of subjects lines please check at least one" class="width100percent">
				<xsl:variable name="field_name"><xsl:value-of select="@name"/></xsl:variable>
				<xsl:for-each select="//xml_document/modules/container/webobject/module/form/emails/option">
	   			<tr><td class="width100percent">
				<label>
					<xsl:attribute name="for">fb_<xsl:value-of select="$field_name"/>_<xsl:value-of select="position()"/></xsl:attribute>
					<xsl:call-template name="get_translation"><xsl:with-param name="check"><xsl:value-of select="." disable-output-escaping="yes" /></xsl:with-param></xsl:call-template>
				</label></td><td>
