...
- Modified $Date: 2004/10/15 15:34:06 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
		...
		<xsl:when test="@type=2">
					   	...
					   	<xsl:choose>
							...
							<xsl:when test="command='WEBOBJECTS_SHOW_SITE_UPDATED_DATE'"><xsl:choose>
								...
								<xsl:otherwise><div><xsl:attribute name='class'><xsl:choose>
									<xsl:when test="$uses_class!=''"><xsl:value-of select="$uses_class"/></xsl:when>
									<xsl:otherwise>date</xsl:otherwise>
								</xsl:choose></xsl:attribute><div class="siteupdatedate"><xsl:value-of select="//setting[@name='site_updated']"/></div></div></xsl:otherwise>
							</xsl:choose></xsl:when>
				...
				<xsl:otherwise>
					<xsl:for-each select="module">
						<xsl:if test="$show_label=1 and $showit=1">
							<div id='label'>
								<xsl:choose>
					   	...
					   	<xsl:choose>
					    	...
					    	<xsl:when test="@name='information_presentation' and @display='ATOZ'">asdf<xsl:choose>
								<xsl:when test="$intable = 1"><xsl:call-template name="display_a_table">
									<xsl:with-param name="header"><xsl:choose><xsl:when test="label"><xsl:value-of select="label"/></xsl:when><xsl:otherwise>Latest Information</xsl:otherwise></xsl:choose></xsl:with-param>
								...
								<xsl:otherwise>
								<xsl:call-template name="display_directory"><xsl:with-param name="uses_class"><xsl:value-of select="$uses_class"/></xsl:with-param></xsl:call-template>
								</xsl:otherwise>
							</xsl:choose></xsl:when>
