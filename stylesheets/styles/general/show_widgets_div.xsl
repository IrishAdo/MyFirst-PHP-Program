...
- Modified $Date: 2005/02/24 22:05:14 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
<xsl:template name="show_webobject_item">
	...
	<xsl:choose>
		...
		<xsl:when test="@type=2">
				<xsl:choose>
					<xsl:when test="boolean(//setting[@name='displaymode']='pda')">
					   	<xsl:choose>
				    		<xsl:when test="module/@display='ATOZ_WIDGET'">
								<xsl:for-each select="module[@display='ATOZ_WIDGET']"><xsl:call-template name="display_a2z_widget"></xsl:call-template></xsl:for-each>
							</xsl:when>
						</xsl:choose>
					</xsl:when>
					<xsl:otherwise>
						<xsl:if test="property/option[name='label']/value!='' and $show_label='1'"><div class='webobjectheader'>
					   	...
					   	<xsl:choose>
							...
							<xsl:when test="command='WEBOBJECTS_SHOW_SEARCH_BOX_FANCY'"><xsl:call-template name="display_fancy_search"><xsl:with-param name="uid"><xsl:value-of select="uid"/></xsl:with-param></xsl:call-template></xsl:when>
				...
				<xsl:otherwise>
					<xsl:for-each select="module">
						<xsl:if test="(//setting[@name='displaymode']!='pda' and $show_label=1 and $showit=1) or ../property/option[name='label']/value!=''">
							<div class='label'>
								<xsl:choose>
