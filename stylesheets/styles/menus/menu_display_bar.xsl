...
<xsl:template name="display_menu">
	<xsl:for-each select="//xml_document/modules/module[@name='layout']">
      <table cellspacing="2" border="0" width="100%" summary="This table contains the menu"><xsl:call-template name="display_menu_parent">
...
<xsl:template name="display_menu_parent">
	...
	<xsl:if test="($found!='') or count(groups/option)=0">
		<tr><td><xsl:attribute name="class"><xsl:choose>		
			<xsl:when test="url=$current_url">currentlocationon</xsl:when>
			<xsl:otherwise>currentlocationoff</xsl:otherwise>
