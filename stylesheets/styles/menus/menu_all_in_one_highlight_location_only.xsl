...
<xsl:template name="display_menu_parent">
	...
	<tr>
		<td><xsl:attribute name='style'>padding-left:<xsl:value-of select="(@depth - 1)* 8 "/>px</xsl:attribute><xsl:attribute name="class"><xsl:choose>
			<xsl:when test="url=$current_url">menuon</xsl:when>
