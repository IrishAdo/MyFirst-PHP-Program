...
- Modified $Date: 2005/01/11 16:36:49 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
<script type="text/javascript" src="/libertas_images/javascripts/web_menu_cewcni2.js"><xsl:comment>Load the Javascript menu</xsl:comment></script>
<script type="text/javascript" >
level_one_image_file = "<xsl:value-of select="$image_path"/>/arrow_rd.gif";
level_one_image_width = "20";
level_one_image_height = "20";
level_x_image_file = "<xsl:value-of select="$image_path"/>/arrow_rd.gif";
level_x_image_width = "20";
level_x_image_height = "20";

beginSTM("Libertas_Solutions","<xsl:value-of select="$dhtml_menu_type"/>","0","0","none","false","true","310","1000","0","100","","/libertas_images/themes/1x1.gif");
	beginfirst(0,0,"<xsl:value-of select="$menu_dhtml_direction"/>",1,level_one_image_file, level_one_image_width, level_one_image_height,"", "", "", "", 1);
	<xsl:for-each select="//xml_document/modules/module[@name='layout']">
		<xsl:for-each select="menu[@hidden=0]">
			...
			<xsl:if test="./children/menu[@hidden=0]">
				begin(<xsl:value-of select="$indent"/>,5,"undefined",0 ,level_x_image_file, level_x_image_width, level_x_image_height, 
				"<xsl:value-of select="$top_colour"/>", "<xsl:value-of select="$bottom_colour"/>", "<xsl:value-of select="$right_colour"/>", 
				"<xsl:value-of select="$left_colour"/>",0);
...
function add_entry_first_level(label_str, url_str, alt_str){
	...
	"Verdana","0.7em","<xsl:value-of select="$colour_text_first_level_off"/>","Normal","normal","none",
	"Verdana","0.7em","<xsl:value-of select="$colour_text_first_level_on"/>","Normal","normal","none",
	"<xsl:value-of select="$menulevel_1_border"/>","solid",
...
function add_entry(label_str, url_str, alt_str){
	...
	"","1","-1","-1","/libertas_images/themes/1x1.gif","/libertas_images/themes/1x1.gif","0","0","0",alt_str,url_str,"_self",
	"Verdana","0.7em","<xsl:value-of select="$colour_text_off"/>","Normal","normal","none",
	"Verdana","0.7em","<xsl:value-of select="$colour_text_on"/>","Normal","normal","none",
	"<xsl:value-of select="$menulevel_n_border"/>","solid",
...
function beginfirst(pos_x, pos_y, direction_required, img, image_file, image_width, image_height, top_colour, bottom_colour, right_colour, left_colour, lvl){
	spacing 		= <xsl:value-of select="$cellspacing"/>;
	padding 		= <xsl:value-of select="$padding"/>;
	shadow_colour 	= "<xsl:value-of select="$shadow_colour"/>";
	shadow_depth 	= <xsl:value-of select="$shadow_depth"/>;
