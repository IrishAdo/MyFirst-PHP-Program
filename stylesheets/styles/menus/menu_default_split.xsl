...
<xsl:template name="display_menu">
	<xsl:param name="split_type">hr</xsl:param>
	<xsl:for-each select="//xml_document/modules/module[@name='layout']">
      <table cellspacing="0" border="0" width="100%" summary="This table contains the menu"><xsl:call-template name="display_menu_parent">
			...
			<xsl:with-param name="split_type"><xsl:value-of select="$split_type"/></xsl:with-param>
	...
	<xsl:if test="(($found!='') or count(groups/option)=0)">
		<tr><td><xsl:attribute name='style'>padding-left:<xsl:value-of select="(@depth - 1)* 8 "/>px</xsl:attribute>
			...
			<xsl:if test="@parent=-1">
				<xsl:choose>
					<xsl:when test="$split_type='img'"><tr><td align="left"><img width="150" height="5"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/hr.gif</xsl:attribute></img></td></tr></xsl:when>
					<xsl:when test="$split_type='hr'"><tr><td align="left"><hr width="95%"/></td></tr></xsl:when>
				</xsl:choose>
			</xsl:if>
    	</xsl:if>
	</xsl:for-each>
