	...
	<xsl:for-each select="//xml_document/modules/module[@name='layout']">
		<xsl:for-each select="menu[@hidden=0]">
			...
			<xsl:if test="./children/menu[@hidden=0]">
				begin(<xsl:value-of select="$indent"/>,0,"undefined",0,level_x_image_file, level_x_image_width, level_x_image_height, "<xsl:value-of select="$top_colour"/>", "<xsl:value-of select="$bottom_colour"/>", "<xsl:value-of select="$right_colour"/>", "<xsl:value-of select="$left_colour"/>",0);
