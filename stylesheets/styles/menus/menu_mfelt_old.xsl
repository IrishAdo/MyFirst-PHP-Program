...
<xsl:template name="display_menu">
    ...
    <table cellspacing="0" border="0" width="100%" summary="This table contains the menu">
	  <tr><td class="menuheader"><img height="10" width="10" src="/libertas_images/themes/1x1.gif" alt=""/></td></tr>
...
<xsl:template name="display_menu_parent">
	...
	<xsl:for-each select="menu[@parent=$parent_identifier and @hidden='0']">
		...
		<xsl:if test="($found!='') or count(groups/option)=0">
			<tr><td>
			<xsl:if test="@depth!=1">
			<xsl:attribute name='style'>padding-left:<xsl:value-of select="10 + ((@depth - 1)* 8) "/>px</xsl:attribute>
			</xsl:if>
