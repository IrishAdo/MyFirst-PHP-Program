...
<xsl:for-each select="menu[@parent=$parent_identifier]">
		...
		<xsl:if test="($found!='') or count(groups/option)=0"><td><xsl:attribute name="background"><xsl:choose>
			...
			<xsl:when test=".//children/menu[url=//setting[@name='script'] and @hidden='0']"><xsl:value-of select="$image_path"/>/menu_bg_on.gif</xsl:when>
		...
		[[nbsp]][[nbsp]]<xsl:call-template name="print"><xsl:with-param name="str_value"><xsl:copy-of select="label" /></xsl:with-param></xsl:call-template>[[nbsp]][[nbsp]]</a></td>
		<td><img width="2" height="20" alt="splitter between menu options"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/button_type_1_splitter.gif</xsl:attribute></img></td></xsl:if></xsl:for-each>
	</tr>
</xsl:template>
				...
				<xsl:when test="url=//setting[@name='script'] and @hidden='0'">
					<xsl:if test="children/menu">
						<xsl:for-each select="children/menu[@hidden='0']">
							<hr/>
								<xsl:if test="url=//setting[@name='script'] and @hidden='0'"><img border="0"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/arrow.gif</xsl:attribute></img></xsl:if>
									<a><xsl:attribute name="class">menulevel1</xsl:attribute>
										<xsl:if test="@accesskey!=''"><xsl:attribute name="accesskey"><xsl:value-of select="@accesskey"/></xsl:attribute></xsl:if>
										<xsl:attribute name="href"><xsl:value-of select="url"/></xsl:attribute>
										<xsl:attribute name="title"><xsl:call-template name="print"><xsl:with-param name="str_value"><xsl:choose>
											<xsl:when test="alt_text!=''"><xsl:value-of select="alt_text" disable-output-escaping="yes"/></xsl:when>
...
<xsl:template name="display_submenu_parent">
	...
	<xsl:for-each select="//children/menu[@parent=$parent_identifier and @hidden='0']">
		...
		<xsl:if test="(//session/groups/group/@identifier = groups/option) or not(groups/option)">
			<xsl:if test="url=//setting[@name='script'] and @hidden='0'"><img border="0" alt="Arrow"><xsl:attribute name="src"><xsl:value-of select="$image_path"/>/arrow.gif</xsl:attribute></img></xsl:if>
			<a><xsl:attribute name="class">menulevel2</xsl:attribute>
				...
				<xsl:call-template name="print"><xsl:with-param name="str_value"><xsl:copy-of select="label" /></xsl:with-param></xsl:call-template></a><xsl:call-template name="display_indent">
