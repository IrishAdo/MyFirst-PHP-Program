...
<xsl:attribute name="class"><xsl:value-of select="$class_to_use"/>bg</xsl:attribute>| 
	...
	<xsl:for-each select="//xml_document/modules/module[@name='layout']">
		<xsl:for-each select="menu">
			...
			<xsl:when test="url=//setting[@name='script'] and @hidden='0'">
				...
				<table border="0" cellspacing="0" cellpadding="0" width="100%" summary="This table contains all sub menu levels of the root location you choose. The First row contains the Root location label and the second row contains the children of that location if any">
					<tr>
					...
					<tr>
						<td><table border="0" cellspacing="0" cellpadding="0" width="100%" summary="This contains the children of the root location that you chose if any exist.">
...
<xsl:template name="display_submenu_parent">
	...
	<xsl:if test="(//session/groups/group/@identifier = groups/option) or not(groups/option)">
		...
		<tr>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" height="20"><tr><td width="15"><img border="0" align="right" src="/iamges/themes/1x1.gif" width="5" height="1"/></td>
			<td>
			<xsl:attribute name='style'>padding-left:<xsl:value-of select="((@depth - 2)* 8)+3 "/>px</xsl:attribute>
