...
*/
/**
* This module is the module for embedded content in a page
*/
class embededinfo extends module{
	...
	*/
	...
	var $module_name_label			= "Embedded Link Management Module (SYSTEM)";
	var $module_admin				= "0";
		...
		*/
		...
		if (strpos($user_command,$this->module_command)===0){
			...
			if ($this->module_admin_access==1){
				if ($user_command==$this->module_command."CACHE_LINKS"){
					return $this->cache_links($parameter_list);
				}
...
";
	...
	function update_embed($parameters){
		...
		$start = $this->check_parameters($parameters,"start",0);
		$id = $this->check_parameters($parameters,"id",0);
		if ($seek_index-500 > $start){
			print "<meta http-equiv='Refresh' content='10;url=index.php?command=EMBED_UPDATE_INFO&start=$seek_index&seek_index=$seek_index'>";
			$this->exitprogram();
		}else {
			if($id==0)
			...
			if ($total > $seek_index){
				...
				$this->call_command("ENGINE_REFRESH_BUFFER",Array("command=EMBED_UPDATE_INFO&start=$start&seek_index=".($seek_index+10)));
			}
		}
	...
	function manage_links($parameters){
		...
		";*/
		if ($debug)	print "<h1>Manage Links</h1>".print_r($parameters,true)."".$sql."";
		$result = $this->call_command("DB_QUERY",array($sql));
		if (!$result){
			if ($this->module_debug){
...
//						print "<p>On field [<strong>".$r["mi_field"]."</strong>]replace [".$url."] with [".$url_to_check."]</p>";
						$p_list = Array(
									"type"			=> "MENU", 
									"embed_id"		=> $embed_id, 
									"replace"		=> $url, 
...
//					}
	...
	*/
	...
	function find_and_fix($parameters){
		$debug = false;
		if ($this->module_debug){
			$this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"find_and_fix",__LINE__,"[".join($parameters,", ")."]"));
		}

		$p_menu_id 	= $this->check_parameters($parameters,"o_menu");
		$c_menu_id	= $this->check_parameters($parameters,"n_menu");
		$prev_path	= $this->check_parameters($parameters,"op");
		$tpath		= $this->check_parameters($parameters,"np");
		$with		= $tpath.$this->make_uri($this->check_parameters($parameters,"current")).".php";
		$replace	= $prev_path.$this->make_uri($this->check_parameters($parameters,"old")).".php";
		if ($this->module_debug){
			$this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"find_and_fix",__LINE__,"[Replace $replace with $with]"));
		}
		if ($replace!=".php"){
			/*
				-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
				...
				*/
			if ($this->parent->server[LICENCE_TYPE]==ECMS || (($this->check_parameters($parameters,"current") != $this->check_parameters($parameters,"old")) && ($p_menu_id == $c_menu_id))){
				$condition = "destination_url like '$replace%' and ";
			} else if (($this->check_parameters($parameters,"current") != $this->check_parameters($parameters,"old")) && ($p_menu_id != $c_menu_id)){
				$condition = "(destination_url like '$replace%' or destination_url like '$with%') and src_mi.mi_memo like '%$replace%' and ";
			} else {
				$condition = "destination_url like '$with%' and src_mi.mi_memo like '%$replace%' and ";
			}
			...
			$sql = "select 
						distinct  
							...
							dst_identifier, 
							destination_url,
							src_mi.mi_memo as src_memo, 
							src_mi.mi_identifier as src_id
					from embed_libertas_link
						inner join memo_information as src_mi on src_identifier = src_mi.mi_link_id and src_mi.mi_type='PAGE_' and src_mi.mi_field= editor
						inner join page_trans_data as ptd1 on src_mi.mi_link_id = ptd1.trans_identifier and ptd1.trans_client= src_mi.mi_client
					where 
						$condition
						client_identifier = $this->client_identifier";
			if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,__FUNCTION__."::SQL",__LINE__,"$sql"));}
			...
			$id_list = Array();
	        while($r = $this->call_command("DB_FETCH_ARRAY",Array($result))){
				$embed_identifier=$r["embed_identifier"];
				$dst_identifier=$r["dst_identifier"];
				if (!in_array($r["src_id"],$id_list)){
					$id_list[count($id_list)] = $r["src_id"];
	        		$memo	= str_replace("$replace", "$with",  $this->check_parameters($r,"src_memo"));
					$sql = "update memo_information set mi_memo='$memo' where mi_client = $this->client_identifier and mi_identifier= ".$r["src_id"];
					if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"SQL",__LINE__,"ff:: replacing ($replace,$with) <br>$sql"));}
					$this->call_command("DB_QUERY",Array($sql));
				}
	        }
			if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"id_list",__LINE__,"ids :: ".print_r($id_list,true).""));}
			
	        $this->call_command("DB_FREE",Array($result));
			$sql = "update embed_libertas_link set menu_identifier='$c_menu_id', destination_url='$with' where destination_url like '$replace%' and client_identifier=$this->client_identifier";
			// and dst_identifier=".$dst_identifier;
			if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"SQL",__LINE__,"ff:: ud link $sql"));}
			$this->call_command("DB_QUERY",array($sql));
		}
	}
	
	function fix_menu($parameters){
		if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"fix_menu",__LINE__,"".print_r($parameters,true).""));}
		...
		$sql = "select 
				...
				from embed_libertas_link
					inner join memo_information as src_mi on src_identifier = src_mi.mi_link_id and src_mi.mi_type='PAGE_' and src_mi.mi_field= editor
					inner join page_trans_data as ptd1 on src_mi.mi_link_id = ptd1.trans_identifier and ptd1.trans_client= src_mi.mi_client
					inner join menu_data on menu_data.menu_identifier = embed_libertas_link.menu_identifier and menu_client = embed_libertas_link.client_identifier
				where 
...
//					dst_identifier = $page and 
        ...
        while($r = $this->call_command("DB_FETCH_ARRAY",Array($result))){
			if (!in_array($r["src_id"],$id_list)){
				...
				$ptd2id 		= $r["ptd2id"];
				$list  			= split('/',$path);
				$replace[count($replace)]= $path;
				$w= $this->call_command("LAYOUT_GET_DIRECTORY_PATH",Array($r["menu_directory"]))."".$list[count($list)-1];
				$with[count($with)]		= $w;
        		$memo					= str_replace($replace, $with,  $this->check_parameters($r,"src_memo"));
				$sql 					= "update memo_information set mi_memo='$memo' where mi_client = $this->client_identifier and mi_identifier= ".$r["src_id"];
				if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"SQL",__LINE__,"fm:: replacing (".print_r($replace,true).",".print_r($with,true).") <br>$sql"));}
				$this->call_command("DB_QUERY",Array($sql));
				$sql 					= "update embed_libertas_link set destination_url='$w' where destination_url like '$path%' and client_identifier=$this->client_identifier and embed_identifier = ".$r["embed_identifier"];
				if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"SQL",__LINE__,"fm:: ud link <br>$sql"));}
				$this->call_command("DB_QUERY",array($sql));
			}
        }
        $this->call_command("DB_FREE",Array($result));
	...
	function fix_pages($parameters){
		if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"fix_pages",__LINE__,"fix_pages :: ".print_r($parameters,true).""));}
...
//		if ($this->parent->server[LICENCE_TYPE]==ECMS || (($this->check_parameters($parameters,"current") != $this->check_parameters($parameters,"old")) && ($p_menu_id == $c_menu_id))){
		if ($this->parent->server[LICENCE_TYPE]==ECMS){
			$condition = "	destination_url like '$replace%' and menu_identifier = $msi and dst_identifier = $page and ";
		} else if (($this->check_parameters($parameters,"current") != $this->check_parameters($parameters,"old")) && ($p_menu_id != $c_menu_id)){
			$condition = "(destination_url like '$replace%' or destination_url like '$with%') and src_mi.mi_memo like '%$replace%' and menu_identifier = $mdi and ";
		} else {
			$condition = "	destination_url like '$with%' and src_mi.mi_memo like '%$replace%' and menu_identifier = $mdi and 
			";
		}
//						dst_identifier = $page and 
		...
		$sql ="select 
				...
				from embed_libertas_link
					inner join memo_information as src_mi on src_identifier = src_mi.mi_link_id and src_mi.mi_type='PAGE_' and src_mi.mi_field= editor
					inner join page_trans_data as ptd1 on src_mi.mi_link_id = ptd1.trans_identifier and ptd1.trans_published_version=1 and ptd1.trans_client= src_mi.mi_client
				where 
					$condition
					src_mi.mi_client = $this->client_identifier";
		if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,__FUNCTION__."::SQL",__LINE__,"$sql"));}
		$result  = $this->call_command("DB_QUERY",Array($sql));
		$id_list = Array();
        while($r = $this->call_command("DB_FETCH_ARRAY",Array($result))){
	...
	function remove_embeded_info($parameters){
		if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"remove_embeded_info",__LINE__,"".print_r($parameters,true).""));}
		...
		$trans_identifier	= $this->check_parameters($parameters,"identifier", -1);
		$trans_id_list="";
...
//		print 
	...
	function cache_links($parameters){
		$dst = $this->check_parameters($parameters,"dst",-1);
//		print_r($parameters);
		if ($dst!=-1){
			$sql = "select src_identifier from embed_libertas_link 
						inner join page_trans_data on src_identifier = trans_identifier and trans_client = client_identifier
					where 
						dst_identifier = $dst and 
			...
			$result  = $this->call_command("DB_QUERY",Array($sql));
            while ($r = $this->call_command("DB_FETCH_ARRAY", Array($result)) ){
				if ($this->module_debug){ $this->call_command("UTILS_DEBUG_ENTRY",array($this->module_name,"cache_links",__LINE__,"[".$r["src_identifier"]."]"));}
            	$this->call_command("PAGE_CACHE_PAGE", Array("identifier"=>$r["src_identifier"] , "set_publish"=>0, "no_to_sublevel"=>"yes") );
