			...
			*/
			for($index=0,$max_length=count($this->modules);$index<$max_length;$index++){
				...
				if ((strpos($user_command,$this->modules[$index]["tag"])===0) && (file_exists(dirname(__FILE__)."/".$this->modules[$index]["file"]))){
					if ($this->modules[$index]["module"]==null){
						if (file_exists(join("/",split("\\\\",dirname(__FILE__)))."/".$this->modules[$index]["file"])){
							...
							$command_eval	.= "\$this->modules[\$index][\"module\"] = new ".$this->modules[$index]["name"]."(\$this,\$parameter_list);";
							eval ($command_eval);
							if($index=="DB_"){
...
<table height='100%'><tr><td valign='top' width='200px' style='background-color:#e7eef5;border-right:1px solid black'>
<ul>
...
/*
		$this->breadcrumb = "Client Management <small>&gt;&gt;</small> Define Connection Details ";	
		$pwd = $this->pwd;
		if (strlen($pwd)>0){
			$pwd = "__STORED_IN_LICENCE__";
		}
		$out  = "
		<h1>Connection details to database</h1>
		<form method=post>
		<input type=hidden name='command' value='INSTALLER_MANAGE_CLIENT_STEP_TWO'>
		<table>
		<tr><td>Host database server</td><td><input type='text' name='host' value='$this->host' size=30 border='0'/></td></tr>
		<tr><td>System database</td><td><input type='text' name='db' value='$this->database' size=30 border='0'/></td></tr>
		<tr><td>Username</td><td><input type='text' name='user' value='$this->user' size=30 border='0'/></td></tr>
		<tr><td>Password</td><td><input type='password' name='pwd' value='$pwd' size=30 border='0'/></td></tr>
		<tr><td colspan=2 align=right><input type='submit' class='bt' value='Connect' border='0'/></td></tr>
		</table>
		</form>
		";
		$this->page_output .= $out;
	...
	function create_client_step_two($parameters){
		$this->breadcrumb = "Client Management <small>&gt;&gt;</small> Define Connection Details <small>&gt;&gt;</small> Testing Connection ";	
		$pwd = $this->pwd;
		$_SESSION["installer_host"] = $this->check_parameters($parameters,"host");
		$_SESSION["installer_user"] = $this->check_parameters($parameters,"user");
		$_SESSION["installer_db"] = $this->check_parameters($parameters,"db");
		$pwd = $this->check_parameters($parameters,"pwd");
		if ($pwd == "__STORED_IN_LICENCE__"){
			$_SESSION["installer_pwd"] = $this->pwd;
		}
		$test = $this->connection();
		if ($test==1){
		$out  = "
		<h1>Testing Connection</h1>
		<p>The Connection details you have submitted are correct</p>
		<p>You can now manage the clients for this database.</p><p><input type=button onclick=\"javascript:window.location='install.php?command=INSTALLER_MANAGE_CLIENT_STEP_THREE&".session_name()."=".session_id()."'\" value='Manage Clients'></p>
		";
		}
		if ($test==-1){
		$out  = "
		<h1>Testing Connection</h1>
		<p>The Connection details you have submitted are incorrect the database specified was not found.</p>
		<p>Please check that the database exists on the server in question and that the user you specified can access the database</p>
		";
		}
		if ($test==-2){
		$out  = "
		<h1>Testing Connection</h1>
		<p>The Connection details you have submitted are incorrect the database server specified was not found.</p>
		<p>Please check that the database host is correct and that the username and password you specified are correct</p>
		";
		}
		$this->page_output .= $out;
	}

	function create_client_step_three($parameters){
*/
		$test = $this->connection();
		if ($test!=1){
...
//			$out = create_db_step_one($parameters);
			...
			if ($action_type=="ADD"){
				...
				$sql = "
					insert into client 
						(client_name, client_contact, client_date_created) 
					values 
						('$company_client',-1,'$now')
				";
				...
				*/
				...
				$this->call_command("DB_QUERY", Array($sql));
				for($index=0,$length=count($this->modules);$index<$length;$index++){
					//print $this->modules[$index]["tag"]."CREATE_NEW_CLIENT_DETAILS::".$this->modules[$index]["table"]."<BR>";
					if ($this->modules[$index]["table"]==1){
