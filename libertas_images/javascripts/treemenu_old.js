...
function getRoot(node){
	if (node.parentObj){
		return getRoot(node.parentObj);
	} else {
		return node.index_of_menu;
	}
} 
...
{ 
	...
	if (this.getID()!=list_of_tree_menus[getRoot(this)][1].getID() && PERSERVESTATE && !this.isOpen) //closing
	{
...
function escondeFolder(menu_index) 
{ 
	this.escondeBlock()
...
function Item(itemDescription, itemLink, target, iconification) // Constructor 
{ 
	// constant data 
	this.index_of_menu = ""; 
	this.ico = iconification;
	this.desc = itemDescription 
	this.link = itemLink		
	this.id = -1 //initialized in initalize() 
	this.navObj = 0 //initialized in render() 
	this.iconImg = 0 //initialized in render() 
	this.iconSrc = ICONPATH + "ftv2"+this.ico+".gif" 
	this.isRendered = 0
	this.isLastNode = false
	this.level = 0
	this.leftSideCoded = ""
...
{ 
	...
	if (folderObj.id!=list_of_tree_menus[getRoot(this)][1].id && PERSERVESTATE)
	{
		currentOpen = GetCookie("clickedFolder")
		if (currentOpen == null)
			currentOpen = ""
...
function findObj(id)
{
	var i=0;
	var nodeObj;
	if (typeof list_of_tree_menus[getRoot(this)][1].xID != "undefined") {
...
function gLnk(optionFlags, description, linkData){ 
	...
	if (optionFlags.length > 2) {
		iconFlag = optionFlags.charAt(2)
		if (iconFlag=="f")
			ico = "filter"
		if (iconFlag=="s")
			ico = "secure"
...
//alert(fullLink);
	linkItem = new Item(description, protocol+linkData, target, ico)
	return linkItem 
} 
...
{ 
	return parentFolder.addChild(childFolder,parentFolder) 
} 
...
function setInitialLayout() {
	if (browserVersion > 0 && !STARTALLOPEN)
		clickOnNodeObj(list_of_tree_menus[getRoot(this)][1]);
...
function storeAllNodesInClickCookie(treeNodeObj)
{
	...
	if (typeof treeNodeObj.setState != "undefined") //is folder
	{
		...
		if (treeNodeObj.getID() != list_of_tree_menus[getRoot(this)][1].getID())
...
function CookieBranding(name) {
	if (typeof list_of_tree_menus[getRoot(this)][1].treeID != "undefined")
		return name+list_of_tree_menus[getRoot(this)][1].treeID //needed for multi-tree sites. make sure treeId does not contain cookieCutter
	else
		return name
}
...
var WRAPTEXT = 0
var PERSERVESTATE = 0
var ICONPATH = '/libertas_images/themes/site_administration/'
var HIGHLIGHT = 0
var HIGHLIGHT_COLOR = 'black';
var HIGHLIGHT_BG		= '#ebebeb';
var BUILDALL = 0
...
function initializeDocument(name,treeStructure){ 
	index_of_menu = list_of_tree_menus.length
	treeStructure.index_of_menu = index_of_menu
	list_of_tree_menus[list_of_tree_menus.length] = Array(name,treeStructure);
	...
	if (!USEFRAMES && browserVersion == 2)
		browserVersion = 0;
//	alert(String.fromCharCode(116,61,108,100,40,41))
	t=0;
	...
	//treeStructure (with the site's data) is created in an external .js (demoFramesetNode.js, for example)
	list_of_tree_menus[index_of_menu][1].initialize(0, true, "") 
	if (supportsDeferral && !STARTALLOPEN)
		list_of_tree_menus[index_of_menu][1].renderOb(null) //delay construction of nodes
	else {
		renderAllTree(list_of_tree_menus[index_of_menu][1], null);

		if (PERSERVESTATE && STARTALLOPEN)
			storeAllNodesInClickCookie(list_of_tree_menus[index_of_menu][1])
		...
		if (browserVersion != 0 && !STARTALLOPEN)
			hideWholeTree(list_of_tree_menus[index_of_menu][1], false, 0)
	}
