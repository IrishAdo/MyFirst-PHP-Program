...
if(nDM&&!nNN4)
{
	var s="<STYLE>\n.st_tbcss,.st_tdcss,.st_divcss,.st_ftcss{cursor:hand;border:none;padding:0px;margin:0px;}\n</STYLE>";
	for(var j=0;j<10;++j)
...
function stm_bm(a)
{
	...
	xalpha= nNN4?Math.max(100,a[11]):a[11];
	st_ms[st_cm]={ps:[],mei:st_cm,ids:"Stm"+st_cm+"p",hdid:0,cked:0,cfrm:0,tfrm:window,sfrm:window,mcff:"",mcfd:0,mcfn:0,mcfb:1,mcfx:0,mcfy:0,mnam:a[0],mver:a[1],mweb:w,mbnk:stbuf(w+a[3]),mtyp:a[4],mcox:a[5],mcoy:a[6],maln:stHAL[a[7]],mcks:a[8],msdv:a[9],msdh:a[10],mhdd:xalpha,mhds:a[12],mhdo:a[13],mhdi:a[14],mpre:p,args:a.slice(0)};
}
function stm_bp(l,a)
{
	var m=st_ms[st_cm],i=m.ps.length?m.ps[st_cp].is[st_ci]:0;
	st_cp=m.ps.length;st_ci=0;
	xpesh= a[9]?a[9]:"Normal";
	xpehd= a[11]?a[11]:"Normal";
	xpsds= nIEW?a[15]:0;
	xpbdc= (!nDM||nNN4)?a[22].split('\s')[0]:a[22];
//	xpbdc= (!nDM||nNN4)?a[22].split(/\s/gi)[0]:a[22];
	m.ps[st_cp]={is:[],mei:st_cm,ppi:st_cp,ids:"Stm"+st_cm+"p"+st_cp+"i",par:i,tmid:0,citi:-1,issh:0,isst:!st_cp&&!m.mtyp,isck:!st_cp&&m.mcks,exed:0,pver:a[0],pdir:a[1],poffx:a[2],poffy:a[3],pspc:a[4],ppad:a[5],plmw:a[6],prmw:a[7],popc:a[8],pesh:xpesh,pesi:a[10],pehd:xpehd,pehi:a[12],pesp:a[13],pstp:a[14],psds:xpsds,pscl:a[16],pbgc:a[17],pbgi:stbuf(stgsrc(a[18],m,0)),pbgr:stREP[a[19]],pbds:stBDS[a[20]],ipbw:a[21],pbdc:xpbdc,args:a.slice(0)};
	var p=m.ps[st_cp];
...
function stm_ai(l,a)
{
	...
	if(a[0]==6)
		...
		quotestr = a[10];
		while (quotestr.indexOf(" ")!=-1){
			quotestr = quotestr.replace('"',"&quot;");
		}
		xitgt = a[8]?a[8]:"_self";
		xitex = a[0]?a[1]:replacestr;
		xiurl = (!a[7]||stabs(a[7]))?a[7]:m.mpre+a[7];
		xxxibgc = (nOP5 && nVER < 7 || !a[26] ) ? a[25] : "transparent";
		xxxxibgc = ( nOP5 && nVER < 7 || !a[24] ) ? a[23]  : "transparent";
		xxibgc = [xxxxibgc, xxxibgc];
		xibgc = (nOP5&&nVER<7&&a[24]&&a[26])? ["transparent","transparent"] : xxibgc;
		xibdc = (!nDM||nNN4)?[a[33].split('\s')[0],a[34].split('\s')[0]]:[a[33],a[34]];
		...
		p.is[st_ci]={
			...
			ibds:stBDS[a[31]],
			ipbw:a[32],
			ibdc:xibdc,itxc:[a[35],a[36]],itxf:[a[37],a[38]],itxd:[stgdec(a[39]),stgdec(a[40])],args:a.slice(0)};
	}
...
function stm_em()
{
	...
	for(var n=nDM?m.ps.length:1,j=0;j<n;++j)
	{
		...
		{
			var i=p.is[k];
			ph+=(p.pver?"<TR style='cursor:hand;' ID="+i.ids+"TR>":"")+stittx(i)+(p.pver?"</TR>":"");
			if(i.ityp!=6)
				mc+="."+i.ids+"TX0{ cursor:hand;"+sttcss(i,0)+"}\n."+i.ids+"TX1{"+sttcss(i,1)+"}\n";
		}
		ph+=stpetx(p);
...
{
	with(i)
	return (ityp!=6?"border-style:"+ibds+";border-width:"+ipbw+"px;border-color:"+ibdc[n]+";"+(!nIEM&&ibgi[n]?"background-image:url("+ibgi[n]+");background-repeat:"+ibgr[n]+";":""):"")+(nIEM&&ityp!=6?"background:"+ibgd[n]+";":"background-color:"+ibgc[n]+";")+"cursor:hand;";
}
function sttcss(i,n)
{
	with(i)
	return "cursor:hand;font:"+itxf[n]+";text-decoration:"+itxd[n]+";"+(!nDM||nNN4||nIE5?"background-color:transparent;color:"+itxc[n]:"");
}
...
function add_admin_options(libertas_dhtml_ml, libertas_al){
	menu = new Array();
	menu_len = libertas_dhtml_ml.length;	
	access_len = libertas_al.length;
	for (m=0; m < menu_len ; m++){
		menu_started=0;
		var opt_len = libertas_dhtml_ml[m][2].length;
		for (opt_pos=0; opt_pos < opt_len; opt_pos++){
			completly_found=0;
			for (a=0; a < access_len; a++){
				if ((libertas_dhtml_ml[m][2][opt_pos][1] + "ALL" == libertas_al[a]) || ("ALL" == libertas_al[a])){ 
					completly_found=1;
				}
 			}
			leaf_len = libertas_dhtml_ml[m][2][opt_pos][3].length;
			if (completly_found==0){
				for (leaf=0; leaf < leaf_len; leaf++){
					libertas_dhtml_ml[m][2][opt_pos][3][leaf][4]=0;
					if (leaf_len>1){
						for (a=0; a < access_len; a++){
							if ((libertas_dhtml_ml[m][2][opt_pos][3][leaf][4]==0) && ((libertas_dhtml_ml[m][2][opt_pos][3][leaf][1] == libertas_al[a]) || (new String("::"+libertas_dhtml_ml[m][2][opt_pos][3][leaf][2]+"::").indexOf(libertas_al[a])>0))){
								libertas_dhtml_ml[m][2][opt_pos][3][leaf][4]=1;
								if (""+menu[libertas_dhtml_ml[m][0]]=="undefined"){
									menu[libertas_dhtml_ml[m][0]]  = new admin_menu();
								}
								if (""+menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]]=="undefined"){
									menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]]=Array();
								}
								pos = menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]].length;
								menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]][pos] = Array(libertas_dhtml_ml[m][2][opt_pos][3][leaf][0], libertas_dhtml_ml[m][2][opt_pos][3][leaf][3]);
							}
 						}
					}
				}
				...
				if (menu[libertas_dhtml_ml[m][0]].children.length==0){
					menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]]=Array();
				}
				pos = menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]].length;
				menu[libertas_dhtml_ml[m][0]].children[libertas_dhtml_ml[m][2][opt_pos][0]][pos] = Array(libertas_dhtml_ml[m][2][opt_pos][0], libertas_dhtml_ml[m][2][opt_pos][2], new admin_menu());
