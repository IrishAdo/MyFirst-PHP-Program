// Ver: 3.76
var nOP=0,nOP5=0,nIE=0,nIE4=0,nIE5=0,nIE6=0,nNN=0,nNN4=0,nNN6=0,nMac=0,nIEM=0,nIEW=0,nSTMENU=0,NS4=0,nVer=0.0;
var firstlevel =1;
...
function beginSTMB(offset,offset_l,offset_t,arrange,arrow,arrow_w,arrow_h,spacing,padding,bg_cl,bg_image,bg_rep,bd_cl,bd_sz,bd_st,trans,spec,spec_sp,lw_max,lh_max,rw_max,rh_max,bg_pos_x,bg_pos_y,ds_sz,ds_color,hdsp,bd_cl_t,bd_cl_r,bd_cl_b,ds_st, lvlone)
{
	if(!ds_sz)		ds_sz=0;
...
function endSTM()
{
	...
	for(mbi=0;mbi<max_l;mbi++)
	{
		...
		var bodyHTML=getBodyTextH(body,1);
		if(body.isstatic)
			bodyHTML="<TABLE width=100% border=\"0\" STYLE='padding:0px;' CELLPADDING=0 CELLSPACING=0"+(menu.flt=='left' ? "" : " align="+menu.flt)+"><TD class=st_tdcss>"+bodyHTML;
		for(iti=0;iti<body.items.length;iti++)
		{
			...
			if(item.type!='sepline')
			{
				menuCSS+="."+item.block+"TX0{ "+getTextCSS(item,0)+"}";
				menuCSS+="."+item.block+"TX1{ "+getTextCSS(item,1)+"}";
			}
		}
...
function getBodyTextH(body, firstlevel)
{
	...
	else
	{
		...
		s+=nTopTb ? "<TABLE width=\"100%\" class=st_tbcss CELLPADDING=0 CELLSPACING=0 " : (nIE ? "<SPAN" : "<DIV")+" class=st_divcss";
		...
		s+="class=st_tbcss CELLSPACING=0 CELLPADDING=0";
		s+=" ID="+body.block+"TB";
		s+=" STYLE='";
		s+=getBodyCSS(body);
		if(!nOP)
