...
// 1.01 27-01-2004  Added break points and stepping (NONE, EACH, and BREAKPOINTS)                                     //
//                                                                                                                    //
//                                                                                                                    //
// Copyright                                                                                                          //
// ---------                                                                                                          //
...
var DEBUG_NONE 			= "__DEBUG_RUNTHROUGH__";
var DEBUG_EACH 			= "__DEBUG_EACH__";
var DEBUG_BREAKPOINTS	= "__DEBUG_BREAKPOINTS__";
/*
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
- default functions to be over 
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
*/
//function debug(){}
//function debug_var(){}
...
if (tw_debug_on+'' == 'undefined'){
	tw_debug_on=false;    
}
tw_debug_float	= false;
tw_debug_ascending	= true;
tw_step_through	= DEBUG_NONE;
...
window.breakpoint	=tw_add_function(window.breakpoint,		'tw_break_point(tw_string)');
window.debug_alert	=tw_add_function(window.debug_alert,    'tw_debug_alert(tw_string, tw_type, tw_cdata)');
// =====================================================================================================================
// tw_debug_onunload
// =====================================================================================================================
//
...
function tw_debug_init(){
  ...
  with (tw_debug_win.document) {
    ...
    writeln('<table cellspacing="0" cellpadding="0" border="0"><tr><td><p style="font: 8pt MS Sans Serif;font-weight:bold;">Debug-Log:</p></td></tr><tr><td><p style="font: 8pt MS Sans Serif;">&nbsp;&nbsp;');      
    if (tw_debug_ascending) writeln('(highest is oldest)');
    else                    writeln('(highest is newest)');
    writeln('</p></td></tr><tr><td>');      
    ...
    writeln('</td></tr><tr><td><p style="font: 8pt MS Sans Serif;font-weight:bold;">Debug-Input:</p></td></tr><tr><td>');      
    ...
    writeln('</td></tr><tr><td><p style="font: 8pt MS Sans Serif;font-weight:bold;">Variable Monitoring:</p></td></tr>');      
    writeln('<tr><td>');      
    ...
    writeln('<br><input type="button" value="close debug console" onClick="opener.tw_debug_close()">');
    writeln('</td></tr></table></form></body></html>');
    close();
  }
  if (!tw_debug_float){
    self.moveTo(0,0);
...
function tw_debug(tw_string,typeOfDebug) {
debug_splitter=":-----------------------------------------:\n";
	if (tw_debug_on){
		if (typeof(tw_debug_win)!='undefined' && tw_debug_win.closed==false){
			if (tw_debug_ascending)
				tw_debug_win.document.forms[0].debug.value+=debug_splitter+tw_string+"\n";
      		else
				tw_debug_win.document.forms[0].debug.value=debug_splitter+tw_string+"\n"+tw_debug_win.document.forms[0].debug.value;
		}
	}
	if(tw_step_through != DEBUG_NONE){
		if (tw_step_through == DEBUG_EACH){
			answer = confirm("Continue to next Debug Statement?")
			if (!answer){
				tw_step_through = DEBUG_BREAKPOINTS;
			}
		}
	}
...
// at a breakpoint set the debug to cycle through all debugs
function tw_break_point(point){
	if (tw_debug_on){
		if (tw_debug_win.document.forms[0].debug.value!=""){
			if(point+""=='undefined'){
				point="";
			}
			if (confirm(point+"\nClear all results from the debug window?")){
				tw_debug_clear_debug();
				tw_debug_clear_monitor();
			}
		}
		tw_step_through = DEBUG_EACH;
	}
}

function tw_debug_alert(str,type,cdata){
	if (tw_debug_on){
		if (type+''!='undefined'){
