...
function LIBERTAS_design_tab_click(editor, sender){
	//start for firefox modified by Muhammad Imran
	if (this[editor+'_rEdit'].currentDesignMode!="On"){
		...
		document.getElementById('LIBERTAS_'+editor+'_editor_mode').value = 'design';

		// turn off html mode toolbars
		document.getElementById('LIBERTAS_'+editor+'_toolbar_top_html').style.display = 'none';
		document.getElementById('LIBERTAS_'+editor+'_TableButtons').style.visibile = 'show';
		document.getElementById('LIBERTAS_'+editor+'_TableButtons').style.display = '';
		// turn on design mode toolbars
		document.getElementById('LIBERTAS_'+editor+'_toolbar_top_design').style.display = 'inline';
		// switch editors		
		document.getElementById(editor).style.display = "none";
		document.getElementById(editor+"_rEdit").style.display = "inline";
//		document.getElementById(editor+"_rEdit").document.body.focus();
	...
	//end for firefox modified by Muhammad Imran			
}
...
function LIBERTAS_html_tab_click(editor, sender){
	...
	if (this[editor+'_rEdit'].currentDesignMode=='On'){
		...
		document.getElementById('LIBERTAS_'+editor+'_editor_mode').value = 'html';

		// turn off design mode toolbars
		document.getElementById('LIBERTAS_'+editor+'_toolbar_top_design').style.display	= 'none';
		document.getElementById('LIBERTAS_'+editor+'_TableButtons').style.visibile = 'hide';
		document.getElementById('LIBERTAS_'+editor+'_TableButtons').style.display = 'none';
		// turn on html mode toolbars
		document.getElementById('LIBERTAS_'+editor+'_toolbar_top_html').style.display		= 'inline';
		// switch editors		
...
function LIBERTAS_movie_click(editor, sender){
	...
	movie_window = window.open('/libertas_images/editor/libertas/dialogs/movie_library.php?lang=' + document.getElementById('LIBERTAS_'+editor+'_lang').value + '&theme=' + document.getElementById('LIBERTAS_'+editor+'_theme').value +'&'+ session_url+'&base_href='+escape(base_href), "popup", "width=500,height=275,toolbars=no,statusbar=yes;");//
	movie_window.focus();
}
...
function LIBERTAS_audio_click(editor, sender){
	myEditor=editor;
	window.frames[editor+'_rEdit'].focus();		 
	myWin = window.open('/libertas_images/editor/libertas/dialogs/audio_library.php?lang=' + document.getElementById('LIBERTAS_'+editor+'_lang').value + '&theme=' + document.getElementById('LIBERTAS_'+editor+'_theme').value +'&'+ session_url+'&base_href='+escape(base_href), "popup", "width=500,height=275,toolbars=no,statusbar=yes;");//
	myWin.focus();
}
...
function LIBERTAS_flash_click(editor, sender){
	myEditor=editor;
	window.frames[editor+'_rEdit'].focus();		 
	image_window = window.open('/libertas_images/editor/libertas/dialogs/flash_library.php?lang=' + document.getElementById('LIBERTAS_'+editor+'_lang').value + '&theme=' + document.getElementById('LIBERTAS_'+editor+'_theme').value +'&'+ session_url+'&base_href='+escape(base_href), "popup", "width=500,height=275,toolbars=no,statusbar=yes;");
	image_window.focus();
}
...
function LIBERTAS_change_paragraph_click(editor, sender, override){
	...
	if (page_ptr.parentElement){
		...
		element_ptr = _Libertas_GetBlock(page_ptr[0]);
		//element_ptr = _Libertas_GetBlock(page_ptr.item(0));
	}
	//for firefox modified by Ali
	window.frames[editor+'_rEdit'].document.execCommand("FormatBlock", false, "<"+val+">");
...
*/
	...
	function _Libertas_GetSelection(edit_name){
		...
		var ed = document.getElementById(myEditor+'_rEdit');
		var oSel = this[myEditor+'_rEdit'].document.getSelection();
		//var oSel = this.selection;
		// for firefox modified by Ali
		if (!oSel) {
			// for firefox modified by Ali
//			oSel		= ed.getSelection().createRange();
//			oSel.type	= ed.getSelection().type;
			//oSel		= this[myEditor+'_rEdit'].document.selection.createRange();
			//oSel.type	= this[myEditor+'_rEdit'].document.selection.type;
//			alert(document.getElementById(myEditor+'_rEdit'));
			oSel		= document.getElementById(myEditor+'_rEdit');
//			oSel.type	= ed.getSelection().type;
	...
	function LIBERTAS_regenerate_keywords_from_all_editors_click(editor, sender){
		...
		myObj.keywordSpace = document.getElementById('displayKeywords');
		// Starts For firefox to open popup window for 'Regenerating Keywords' Added By Muhammad Imran
		
		path = '/libertas_images/editor/libertas/dialogs/regenerate_keywords.php?lang=' + document.getElementById('LIBERTAS_'+libertas_editors[0]+'_lang').value + '&theme=' + document.getElementById('LIBERTAS_'+libertas_editors[0]+'_theme').value +'&'+ session_url+'&base_href='+escape(base_href);
...
//	movie_window = window.open('/libertas_images/editor/libertas/dialogs/movie_library.php?lang=' + document.all['LIBERTAS_'+editor+'_lang'].value + '&theme=' + document.all['LIBERTAS_'+editor+'_theme'].value +'&'+ session_url+'&base_href='+escape(base_href), "popup", "width=500,height=275,toolbars=no,statusbar=yes;");//
		...
		var keys = window.open(path, myObj, "height=220, width=500, resizable=no, statusbar=no;");

		// ends For firefox to open popup window for 'Regenerating Keywords' Added By Muhammad Imran

		document.getElementById('displayKeywords').innerHTML = keys;
	}
/*
...
function LIBERTAS_set_font_size(editor, sender) {
	var selection = range(editor);
	//for firefox modified by Ali
	var noHtml 				= (selection.indexOf("<")==-1)?1:0;
	//var noHtml 				= (selection.htmlText.indexOf("<")==-1)?1:0;
	//for firefox modified by Ali
	var selTypeStartIsTag = (selection.indexOf("<")==2)?1:0;
	//var selTypeStartIsTag = (selection.htmlText.indexOf("<")==2)?1:0;
	//for firefox modified by Ali
	var selTypeEndIsTag = (selection.lastIndexOf(">")==(selection.length-1))?1:0;
	//var selTypeEndIsTag = (selection.htmlText.lastIndexOf(">")==(selection.htmlText.length-1))?1:0;
	try{
		...
		if ((v!="") && (v+""!="undefined")){
			//for firefox modified by Ali
			window.frames[editor+'_rEdit'].document.execCommand("FontSize",false,v);
...
function LIBERTAS_set_font_face(editor, sender) {
	...
	try{
		...
		if ((v!="") && (v+""!="undefined")){
			//for firefox modified by Ali
			window.frames[editor+'_rEdit'].document.execCommand("FontName",false,v);
...
//				im.align 		= (niProps.align)?niProps.align:'';
				...
				if (Align=="left" || Align=="right"){
					...
					im.style.elements['float'] = Align;
				} else {
					//im.style.float 	= "";
					im.style.elements['float'] = "";
				}
...
function __insert_image(imgSrc){
	if(imgSrc != null){
		...
		if (__setAttribute(imgSrc.styleDefinitionfloat+"")){
			el.style.elements['float'] = imgSrc.styleDefinitionfloat;
		}
...
//		alert('LIBERTAS_table_create_click'+window.frames[editor+'_rEdit'].getSelection().type);
//		alert('LIBERTAS_table_create_click'+window.frames[editor+'_rEdit'].document.selection.type);
		if (window.frames[editor+'_rEdit'].getSelection().type != "Control")
		{
			// selection is not a control => insert table 
			access_uri ='/libertas_images/editor/libertas/dialogs/table_gecko.php?lang=' + document.getElementById('LIBERTAS_'+editor+'_lang').value + '&theme=' + document.getElementById('LIBERTAS_'+editor+'_theme').value +'&'+ session_url+'&base_href='+escape(base_href)+'&wai_compliance='+wai_compliance+'&colour_definition='+has_function(editor,'tables_colour');
			var n = new Object();
			if (rows+""!="undefined"){
				n.createRows = rows;
			}
			...
			var nt = window.open(access_uri, n, "height=470px, width=426px, resizable=no, statusbar=no;");
			nt.focus();
/*			//alert(nt.captiontxt);
			if (nt){
//				window.location.reload();
//				window.frames[editor+'_rEdit'].reload(); 

//window.frames[editor+'_rEdit'].focus(); 
//this[editor+'_rEdit'].document.execCommand('bold', false, null); 
//sapw_update_toolbar(editor, true); 
				//oCaption = document.newtable.createCaption();
...
//	window.frames[editor+'_rEdit'].focus();		 
				//alert('ddd');
				//window.frames[editor+'_rEdit'].contentDocument.focus();
				//alert(editor);
			//	document.getElementById(editor).focus();
...
//		var ntProps = window.open(access_uri)//, tProps, 'dialogHeight:250px; dialogWidth:366px; resizable:no; status:no');	
	...
	function makeTable(rows, cols) {
		...
		str = "<table style=\"table-layout:fixed;border-style:solid; \" "; 
