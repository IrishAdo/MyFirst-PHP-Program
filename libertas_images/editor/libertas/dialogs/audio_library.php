...
<P id=tableProps CLASS=tablePropsTitle><img src='/libertas_images/editor/libertas/lib/themes/default/img/tb_audio.gif'/> Embedding Audio Files Manager</P>

<table width='450px' border=0>
	<tr><td><strong>Select audio file</strong></td><td><select id='imagesrc' name='imagesrc'  style='width:230px' onChange="change_properties()"><option>Loading Audio list...</option></select></td></tr>
	...
	<tr id="propRow"><td >Properties:: </td><td id='propCell'></td></tr>
...
<INPUT class="bt" ONCLICK="window.close()" TYPE=reset ID=idCancel VALUE="<?php echo LOCALE_CANCEL;?>">
	...
	<Script>
		...
		get_audio();
		window.opener.__extract_information('audio');
		var window_data ="";
		change_properties()
	function get_audio(){
		if (window.opener.cache_data.frmDoc.audio_data.value!=''){
			if (window.opener.cache_data.frmDoc.audio_data.value!='__NOT_FOUND__'){
				window_data  			= new String(window.opener.cache_data.frmDoc.audio_data.value);
				myArray 		= window_data.split("|1234567890|")
				l				= myArray.length-1;
				list			= "";
				filter_list 	= new Array();
				document.image_browser.imagesrc.options.length=0
//				document.all.image_browser.imagesrc.options[0] = new Option("--- Select an audio file ---");
				for (i = 0;i<l;i+=2){
...
//				alert("Sorry there are no audio available to embed");
			}
		} else {
			setTimeout("get_audio()",1000);
		}
	}
    function selectClick(){
	  	if (document.image_browser.imagesrc.selectedIndex>0){
			audio_info = ''+document.image_browser.imagesrc.options[document.image_browser.imagesrc.selectedIndex].value+"::"+check('controller')+"::"+check('autoplay');
    		window.opener.__insert_audio(audio_info);
        	window.close();
		} else {
...
function filter_file(){
	myFilenameExt = document.all.image_browser.filter_files.options[document.all.image_browser.filter_files.selectedIndex].value;
	f = window.opener.cache_data.frmDoc.file_data.value;
	found			= 0;
	myArray 		= window_data.split("|1234567890|");
	len				= myArray.length-1;
	document.all.image_browser.imagesrc.options.length=0
	document.all.image_browser.imagesrc.options[0] = new Option("--- Select a File to link to ---");
	for (i = 0;i<len;i+=2){
		if (myArray[i+1].split("::")[0].split(".")[1]==myFilenameExt || ''==myFilenameExt)
			document.all.image_browser.imagesrc.options[document.all.image_browser.imagesrc.options.length] = new Option(convert_special_characters(myArray[i],false), myArray[i+1], "");
	}
		
}
function change_properties(){
	myFilenameExt = document.all.image_browser.filter_files.options[document.all.image_browser.filter_files.selectedIndex].value;
	propCell.innerHTML = "<table>"
						+	"<tr>"
						+		"<td>Controller</td><td><input type='checkbox' id='controller' value='true' checked/></td>"
						+		"<td>Auto Play</td><td><input type='checkbox' id='autoplay' value='true' checked/></td>"
						+	"</tr>"
						+ "</table>";
...
function check(identifier){
	eval ("testVal = document.all."+identifier+".checked");
	if (testVal){
		return "true";
	} else {
		return "false";
