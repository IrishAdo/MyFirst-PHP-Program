...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
-->  
<xsl:variable name="image_path">/libertas_images/themes/balmoraltwo</xsl:variable>	 	
...
<xsl:variable name="show_email_friend">Yes<xsl:choose>
	<xsl:when test="//xml_document/modules/module[@name='client']/licence/product/@type='SITE'">No</xsl:when>
	<xsl:otherwise>Yes</xsl:otherwise>
</xsl:choose></xsl:variable>
<xsl:variable name="show_add_bookmark">Yes</xsl:variable>
...
<xsl:variable name="type_of_form">fancy_column</xsl:variable>
</xsl:stylesheet>
