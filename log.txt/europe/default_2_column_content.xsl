...
- Modified $Date: 2005/02/28 17:16:01 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
-->
<xsl:include href="../../styles/pages/display_2_column_plus_content.xsl"/>

<xsl:template match="/">
	<xsl:comment> style sheet 2 column content.xsl </xsl:comment>
