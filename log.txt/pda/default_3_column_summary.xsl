...
- Modified $Date: 2005/02/28 17:19:30 $
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
*/
-->
...
-->
<xsl:include href="../../styles/pages/display_3_column_plus_summary.xsl"/>

<xsl:template match="/">
	<xsl:comment> style sheet 3 column summary.xsl </xsl:comment>
