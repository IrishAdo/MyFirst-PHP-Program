...
<xsl:if test="@name!='USERS_SHOW_LOGIN'">
	...
	<xsl:if test="//textarea[@type='RICH-TEXT']">
		...
		<xsl:choose>		
			...
			<!--
				If browser is other than IE like FF (By Muhammad Imran)
			-->
			<xsl:otherwise>				
			...
			<xsl:choose>
    			<xsl:when test="@size>40 or not(@size)">
   				...
   				<xsl:otherwise>
	   				...
	   				<xsl:attribute name="maxlength"><xsl:choose><xsl:when test="@length"><xsl:value-of select="@length"/></xsl:when><xsl:otherwise><xsl:value-of select="@size"/></xsl:otherwise></xsl:choose></xsl:attribute>
	   			</xsl:otherwise>
			</xsl:choose>
